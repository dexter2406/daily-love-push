...
def send_message(to_user, access_token, city_name, weather, max_temperature, min_temperature, pipi, lizhi, pop, tips,
    ...
    data = {
        ...
        "data": {
            "date": {
                ...
                "color": color_dict.get("date")
            },
            "city": {
                "value": city_name,
                "color": color_dict.get("city")
            },
            "weather": {
                "value": weather,
                "color": color_dict.get("weather")
            },
            "min_temperature": {
                "value": min_temperature,
                "color": color_dict.get("min_temperature")
            },
            "max_temperature": {
                "value": max_temperature,
                "color": color_dict.get("max_temperature")
            },
            "love_day": {
                "value": love_days,
                "color": color_dict.get("love_day")
            },
            "note_en": {
                "value": note_en,
                "color": color_dict.get("note_en")
            },
            "note_ch": {
                "value": note_ch,
                "color": get_color()
    ...
    for i, (key, value) in enumerate(birthdays.items()):
        ...
        # 将生日数据插入data
        data["data"][key] = {"value": birth_day, "color": color_dict.get(f'birthday{i+1}')}
    headers = {
        'Content-Type': 'application/json',
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) '
                      'AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Safari/537.36'
